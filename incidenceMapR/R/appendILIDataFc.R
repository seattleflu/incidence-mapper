...
appendILIDataFc <- function(db, currentWeek){
  ...
  ILI_forecast_dat <- ILI_forecast_dat %>% dplyr::select(encountered_week, ILI_all) %>% dplyr::rename(CDC_ILI = ILI_all)
  
  # logit for covariate
  ILI_forecast_dat$CDC_ILI <- boot::logit(ILI_forecast_dat$CDC_ILI)
