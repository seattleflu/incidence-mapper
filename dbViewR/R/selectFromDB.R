...
selectFromDB <- function( queryIn = jsonlite::toJSON(
  ...
  db$pathogen[db$pathogen %in% c('APZTD4A','S. pneumoniae_APZTD4A','S. pneumoniae') ] <- 'S.pneumoniae'
  db$pathogen[db$pathogen %in% c('AI5IRK5','M. pneumoniae_AI5IRK5','M. pneumoniae') ] <- 'M.pneumoniae'
  db$pathogen[db$pathogen %in% c('C. pneumoniae_AI1RW2H') ] <- 'C.pneumoniae'
  db$pathogen[db$pathogen %in% c('AP7DPVF','EnterovirusA_B 1_AP7DPVF','Ev_pan') ] <- 'EV_pan'
